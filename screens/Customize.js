...
import { View, Text, ScrollView, TouchableOpacity, ToastAndroid, Keyboard } from "react-native"
...
const Customize = () => {
    ...
    const add = (catagory) =>{
        if(!catagories.includes(catagory)){
            ...
            ToastAndroid.show("Catagory already exists",ToastAndroid.TOP)
        Keyboard.dismiss()    
        setText("")    
    }
    ...
    return(
        <View>
            <Modal isOpen={visible} onClose={() => setVisible(prev => !prev)} size="lg">
                ...
                <Input value={text} onChangeText={(txt) => setText(txt)} variant="outline" size={"sm"} placeholder="New Task"></Input>
                <Button w={"100%"} alignSelf={"center"} onPress={() => add(text)}>Add</Button>
                </Modal.Body>
                </Modal.Content>
            </Modal>
            <Button top={200} size={"md"} onPress={() => setVisible(prev => !prev)}>Customize Catagories</Button>
        </View>
    )
}
